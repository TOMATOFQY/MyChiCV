...
*负责一个支持多访问协议的，基于 nvme 的，键值分离的分布式键值数据库的开发。* _该数据库主要功能是作为一个缓存中间件，将用户访问的热点数据缓存至固态硬盘上，加速训练平台的训练效率。_
...
#cventry(
  tl: "2022 OceanBase 数据库大赛" + "   " + iconlink("https://github.com/cmu-db/bustub") +  "   " + "季军 (rank 4/1180)",
  tr: "2022/10 - 2023/01"
)[
初赛内容为拓展 miniob 的功能,使其成为功能齐备的数据库。负责数据库页缓冲区、索引等存储层功能的开发。
#linebreak()
*复赛内容为在给定计算资源限制下，为 OceanBase 优化旁路导入功能。最终优化性能相较于 OceanBase 原 batch insert 方案实现了 10x 的提升。*
- 负责旁路导入中文件解析、压缩算法、归并排序算法、csv2sstable 算法等模块的设计与实现。
- 充分使用调优技术捕获大小优化点加以改进。
- 充分使用池化技术、多线程并行处理技术在架构上做改进以优化效率。 #iconlink("https://github.com/cmu-db/bustub",icon:zhihu)
]
...
#cventry(
  tl: "Sourcetrail Golang Indexer  " + emph("开源贡献   ") + iconlink("https://github.com/cmu-db/bustub", icon:github),
  tr:  "2021/01 - 2021/06"
)[
